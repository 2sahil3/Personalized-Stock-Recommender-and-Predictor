...
def stock_data(ticker_name):
    ...
    dataframe,ltp = data(ticker_name)
...
def sell_stock():
    ...
    if update_portfolio_sell(user, symbol, quantity, ltp):
        ...
        new_capital = int(user['capital']) + total_sale
...
def get_user_portfolio():
    ...
    if not username:
        return jsonify({'error': 'Username is required in the query parameter'}), 400
